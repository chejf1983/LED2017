...
CContJydzComm::CContJydzComm(void)
{
	m_strCOM = "COM1";
	m_iBaudRate = 9600;
	m_ser.m_ucDeviceAddress = 0xBF;
...
void CContJydzComm::SetAddress(BYTE ucPCAddress, BYTE ucDeviceAddress)
{
...
void CContJydzComm::Initialize(CString strCOM, int iBaudRate)
{
	m_strCOM = strCOM;
...
BOOL CContJydzComm::SwitchAllChannel(BOOL bOn)
{
	BOOL bChannel[SAFT1_CHANNEL_NUM];
	for (int i = 0; i < SAFT1_CHANNEL_NUM; i++)
	{
		bChannel[i] = bOn;
	}
...
BOOL CContJydzComm::SwitchOneChannel(int iChannel, BOOL bOn)
{
	ASSERT(iChannel >= 0 && iChannel < SAFT1_CHANNEL_NUM);
...
BOOL CContJydzComm::SetChannel(BOOL bChannel[])
{
	HANDLE comHandle;
