...
CContNyyComm::CContNyyComm(void)
{
	m_strCOM				= "COM1";
	m_iBaudRate				= 9600;
...
void CContNyyComm::SetAddress(BYTE ucPCAddress, BYTE ucDeviceAddress)
{
	m_ser.m_ucPCAddress = ucPCAddress;
	m_ser.m_ucDeviceAddress = ucDeviceAddress;
}

void CContNyyComm::Initialize(CString strCOM, int iBaudRate)
{
	m_strCOM = strCOM;
	m_iBaudRate = iBaudRate;
}

BOOL CContNyyComm::SwitchAllChannel(BOOL bOn)
{
	BOOL bChannel[SAFT2_CHANNEL_NUM];
	for (int i = 0; i < SAFT2_CHANNEL_NUM; i++)
	{
		bChannel[i] = bOn;
	}
...
BOOL CContNyyComm::SwitchOneChannel(int iChannel, BOOL bOn)
{
	ASSERT(iChannel >= 0 && iChannel < SAFT2_CHANNEL_NUM);
...
BOOL CContNyyComm::SetChannel(BOOL bChannel[])
{
	HANDLE comHandle;
