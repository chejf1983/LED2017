...
CThunderRelayComm::CThunderRelayComm(void)
{
	m_strCOM = "COM1";
	m_iBaudRate = 9600;
...
void CThunderRelayComm::SetAddress(BYTE ucPCAddress, BYTE ucDeviceAddress)
{
...
void CThunderRelayComm::Initialize(CString strCOM, int iBaudRate)
{
	m_strCOM = strCOM;
...
BOOL CThunderRelayComm::SwitchAllChannel(BOOL bOn)
{
	BOOL bChannel[THUNDER_CHANNEL_NUM];
	for (int i = 0; i < THUNDER_CHANNEL_NUM; i++)
	{
		bChannel[i] = bOn;
	}
...
BOOL CThunderRelayComm::SwitchOneChannel(int iChannel, BOOL bOn)
{
	ASSERT(iChannel >= 0 && iChannel < THUNDER_CHANNEL_NUM);
...
BOOL CThunderRelayComm::SetChannel(BOOL bChannel[])
{
	HANDLE comHandle;
