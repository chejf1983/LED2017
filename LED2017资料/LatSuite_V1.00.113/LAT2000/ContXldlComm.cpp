...
CContXldlComm::CContXldlComm(void)
{
	m_strCOM = "COM1";
	m_iBaudRate = 9600;
...
void CContXldlComm::SetAddress(BYTE ucPCAddress, BYTE ucDeviceAddress)
{
...
void CContXldlComm::Initialize(CString strCOM, int iBaudRate)
{
	m_strCOM = strCOM;
...
BOOL CContXldlComm::SwitchAllChannel(BOOL bOn)
{
	BOOL bChannel[SAFT3_CHANNEL_NUM];
	for (int i = 0; i < SAFT3_CHANNEL_NUM; i++)
	{
		bChannel[i] = bOn;
	}
...
BOOL CContXldlComm::SwitchOneChannel(int iChannel, BOOL bOn)
{
	ASSERT(iChannel >= 0 && iChannel < SAFT3_CHANNEL_NUM);
...
BOOL CContXldlComm::SetChannel(BOOL bChannel[])
{
	HANDLE comHandle;
