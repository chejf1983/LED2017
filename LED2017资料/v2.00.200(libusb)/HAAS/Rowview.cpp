...
void CRowView::OnBeginPrinting(CDC* pDC, CPrintInfo* pInfo)
{
	...
	CalculateRowMetrics(pDC);
	m_nRowsPerPrintedPage = (int)(nPageHeight*0.92f) / m_nRowHeight;
...
void CRowView::OnPrint(CDC* pDC, CPrintInfo* pInfo)
{
	...
	pDC->SetViewportOrg(pInfo->m_rectDraw.Width()*1/20, -yTopOfPage+pInfo->m_rectDraw.Height()*2/30);
	...
	CRect rectClip = CRect(0, yTopOfPage, nPageWidth,
		 yTopOfPage + m_nRowsPerPrintedPage * m_nRowHeight+10);  //?+10
