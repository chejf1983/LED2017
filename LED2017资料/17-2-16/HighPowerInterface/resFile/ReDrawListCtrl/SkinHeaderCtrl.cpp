...
void CSkinHeaderCtrl::OnPaint() 
{
	...
	memDC.FillSolidRect(&rect, RGB(230,228,231));
	...
	case 0:
		...
		break;
	case 1:
		bitmapSpan.LoadBitmap(IDB_ColHeader2);
		break;
	}

	CBitmap* pOldBitmapSpan = bitmapDC.SelectObject(&bitmapSpan);

	memDC.StretchBlt(rect.left+2, 0, rect.Width(), rect.Height(), &bitmapDC, 0, 0, 1, 12, SRCCOPY);
	...
	{
	case 0:
		bitmap.LoadBitmap(IDB_COLUMNHEADER_START0);
		bitmap2.LoadBitmap(IDB_COLUMNHEADER_SPAN);
		bitmap3.LoadBitmap(IDB_COLUMNHEADER_END0);
		break;
	case 1:
		bitmap.LoadBitmap(IDB_COLUMNHEADER_START0);
		bitmap2.LoadBitmap(IDB_ColHeader2);
		bitmap3.LoadBitmap(IDB_COLUMNHEADER_END0);
		break;
	}
...
//			memDC.BitBlt(rect.left,rect.top,2,12,&bitmapDC,0,0,SRCCOPY);
			memDC.StretchBlt(rect.left, rect.top, 2, rect.Height(), &bitmapDC, 0, 0, 1, 12, SRCCOPY);
		}
		else
		{
			pOldBitmap = bitmapDC.SelectObject(&bitmap2);
//			memDC.BitBlt(rect.left+1,rect.top,1,12,&bitmapDC,0,0,SRCCOPY);
			memDC.StretchBlt(rect.left+1, rect.top, 1, rect.Height(), &bitmapDC, 0, 0, 1, 12, SRCCOPY);
		}
...
//		memDC.BitBlt((rect.right-2), 0, 2, 12, &bitmapDC,0,0,SRCCOPY);
		memDC.StretchBlt((rect.right-2), rect.top, 2, rect.Height(), &bitmapDC, 0, 0, 2, 12, SRCCOPY);
		...
		lf.lfHeight = 18;         /////字体高度
		strcpy(lf.lfFaceName, "Sevenet 7");   //////字体
